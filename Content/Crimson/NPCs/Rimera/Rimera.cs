...
namespace CalamityVanilla.Content.Crimson.NPCs.Rimera;

public class Rimera : ModNPCWithBanner
{
    ...
    public override void AI()
    {
        ...
        {
            Dust d = Dust.NewDustDirect(NPC.position, NPC.width, NPC.height, ModContent.DustType<SnowCrimsonGibs>());
            d.velocity *= 0.2f;
            d.velocity += NPC.velocity * 0.2f;
        }
    ...
    {
        int type = ModContent.DustType<SnowCrimsonGibs>();
        if (NPC.life > 0)
        {
    ...
    {
        var dropRules = Main.ItemDropsDB.GetRulesForNPCID(NPCID.Crimera, false);
    ...
    {
        return Main.hardMode && spawnInfo.Player.ZoneSnow && spawnInfo.Player.ZoneCrimson ? 0.2f : 0f;
    }
}