...
public class SyncedOnHitGlobalItem : GlobalItem
{
    public override void OnHitNPC(Item item, Player player, NPC target, NPC.HitInfo hit, int damageDone)
    {
        if (item.ModItem is ISyncedOnHitEffect i)
        {
            ...
            ISyncedOnHitEffect.SendPacket(true, item.type, player, target, hit.Crit, hit.HitDirection);
        }
    }
}
public class SyncedOnHitGlobalProjectile : GlobalProjectile
{
    public override void OnHitNPC(Projectile projectile, NPC target, NPC.HitInfo hit, int damageDone)
    {
        if (projectile.ModProjectile is ISyncedOnHitEffect i)
        {
            ...
            ISyncedOnHitEffect.SendPacket(false, projectile.identity, Main.player[projectile.owner], target, hit.Crit, hit.HitDirection);
        }
    }
}